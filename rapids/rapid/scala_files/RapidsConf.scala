...
object RapidsConf {
  ...
  // jgh
  val MODI_OPTIMIZER_ENABLED = conf("spark.rapids.sql.modi.optimizer.enabled")
    .internal()
    .doc("Enable cost-based optimizer that will attempt to avoid " +
      "transitions to GPU for operations that will not result in improved performance " +
      "over CPU for dStream Project")
    .booleanConf
    .createWithDefault(false)

  // jgh
  val LMSTREAM_OPTIMIZER_ENABLED = conf("spark.rapids.sql.modi.lm.optimizer.enabled")
    .internal()
    .doc("Enable cost-based optimizer that is devised by LMStream in modi_optimizer")
    .booleanConf
    .createWithDefault(false)
  ...
  // jgh
  val MODI_OPTIMIZER_CLASS_NAME = conf("spark.rapids.sql.modi.optimizer.className")
    .internal()
    .doc("Optimizer implementation class name. The class must implement the " +
      "com.nvidia.spark.rapids.Optimizer trait for dStream")
    .stringConf
    .createWithDefault("com.nvidia.spark.rapids.ModiCostBasedOptimizer")
...
class RapidsConf(conf: Map[String, String]) extends Logging {
  ...
  // jgh
  lazy val modiOptimizerEnabled: Boolean = get(MODI_OPTIMIZER_ENABLED)

  // jgh
  lazy val lmOptimizerEnabled: Boolean = get(LMSTREAM_OPTIMIZER_ENABLED)
  ...
  // jgh
  lazy val modiOptimizerClassName: String = get(MODI_OPTIMIZER_CLASS_NAME)

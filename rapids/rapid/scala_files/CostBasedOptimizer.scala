...
class CostBasedOptimizer extends Optimizer with Logging {
  ...
  def optimize(conf: RapidsConf, plan: SparkPlanMeta[SparkPlan]): Seq[Optimization] = {
    ...
    // jgh start
    var currentMBSize: Double =  -1.0
    currentMBSize = SparkPlan.getCurrentMBSize
    var totalMBSize: Double = -1.0
    totalMBSize = SparkPlan.getTotalMBSize
    var numDSOfMB: Int = -1
    numDSOfMB = SparkPlan.getNumDSOfMB
    logInfo(s"SparkPlan.get currentMBSize : ${currentMBSize}")
    logInfo(s"SparkPlan.get totalMBSize : ${totalMBSize}")
    logInfo(s"SparkPlan.get numDSOfMB : ${numDSOfMB}")
    // jgh end
    ...
    // sej start
    // logInfo(s"[DyOp_sej] childPlans of p : ${plan.childPlans}")
    // logInfo(s"[DyOp_sej] visit result : ${plan.childPlans.map(RowCountPlanVisitor.visit)}")
    logInfo(s"[DyOp_sej] estimatedOutputRows: ${plan.estimatedOutputRows}")
    // sej end
    optimizations
  }
  ...
  private def recursivelyOptimize(
    ...
    plan.estimatedOutputRows = RowCountPlanVisitor.visit(plan)
    logInfo(s"[DyOp_sej] recursively estimatedOutputRows: ${plan.estimatedOutputRows}")
    // sej start
    // logInfo(s"[DyOp_sej] pre-childPlans of p : ${plan.childPlans}")
    // logInfo(s"[DyOp_sej] pre-visit result : ${plan.childPlans.map(RowCountPlanVisitor.visit)}")
    // logInfo(s"[DyOp_sej] pre-estimatedOutputRows: ${plan.estimatedOutputRows}")
    // sej end
    ...
    // sej start
    // logInfo(s"[DyOp_sej] post-childPlans of p : ${plan.childPlans}")
    // logInfo(s"[DyOp_sej] post-visit result : ${plan.childPlans.map(RowCountPlanVisitor.visit)}")
    // logInfo(s"[DyOp_sej] post-estimatedOutputRows: ${plan.estimatedOutputRows}")
    // sej end

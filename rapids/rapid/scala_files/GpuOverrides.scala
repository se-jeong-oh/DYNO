...
object GpuOverrides extends Logging {
  ...
  private def getOptimizations(wrap: SparkPlanMeta[SparkPlan],
    ...
    else if (conf.modiOptimizerEnabled) {
        // we need to run these rules both before and after CBO because the cost
        // is impacted by forcing operators onto CPU due to other rules that we have
        wrap.runAfterTagRules()
        val optimizer = try {
          new ModiCostBasedOptimizer(conf)
        } catch {
          case e: Exception =>
            throw new RuntimeException(s"Failed to create optimizer " +
              s"${conf.modiOptimizerClassName}", e)
        }
        optimizer.optimize(conf, wrap)
    } else {
      Seq.empty
...
case class GpuOverrides() extends Rule[SparkPlan] with Logging {
    ...
    if (conf.isSqlEnabled && conf.isSqlExecuteOnGPU) {
      GpuOverrides.logDuration(conf.shouldExplain,
        ...
        if (conf.logQueryTransformations) {
          ...
          // jgh: logWarning to logInfo
          logInfo(s"${logPrefix}Transformed query:" +
            s"\nOriginal Plan:\n$plan\nTransformed Plan:\n$updatedPlan")
        }
        updatedPlan
      }
  ...
  private def applyOverrides(plan: SparkPlan, conf: RapidsConf): SparkPlan = {
    logInfo(s"[Rapids_jgh] SparkPlan : ${plan.getClass.getSimpleName}") // jgh

    val wrap = GpuOverrides.wrapAndTagPlan(plan, conf)
//    val childplan: Seq[String] = wrap.childPlans.map(_.wrapped.getClass.getSimpleName) // jgh
//    logInfo(s"[Rapids_jgh] wrap's name : ${wrap.wrapped.getClass.getSimpleName} \n" +
//      s"wrap's parent : ${wrap.parent.map(_.wrapped.getClass.getSimpleName)} \n" +
//      s"wrap's childPlans : ${childplan} \n" +
//      s"wrap's Class Type : ${wrap.getClass.getSimpleName}") // jgh
